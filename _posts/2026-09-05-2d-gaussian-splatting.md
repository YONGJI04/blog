...
지용님이 작업하시는 것처럼 3D 재구성 결과를 CAD와 정합(registration)하려면 점구름이나 메쉬가 실제 표면에 가깝게 정렬돼 있어야 하는데, 3DGS 원본은 이 용도에는 기하학적으로 부정확했다 — 2DGS가 풀려는 문제가 바로 이 지점이다.
...
- 개인적으로는 이 논문이 가장 실용적으로 와닿았는데, 진행 중인 Scan-to-CAD 파이프라인(S²-2DGS, IEIE 투고)이 바로 이 2DGS를 베이스로 SAM 마스크를 학습 단계의 supervision으로 추가 확장한 것이기 때문이다. 3DGS 대신 2DGS를 고른 이유가 정확히 이 논문에서 다룬 "재구성 결과를 CAD처럼 명확한 표면 형상과 비교해야 하는" 상황과 맞아떨어진다는 걸 다시 확인했다.
