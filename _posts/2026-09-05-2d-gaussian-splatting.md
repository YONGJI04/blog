...
- 개인적으로 관심 있는 Scan-to-CAD 같은 3D 정합 작업 관점에서 가장 실용적으로 와닿은 논문이었다. 재구성 결과를 CAD처럼 명확한 표면 형상과 비교해야 하는 상황이라면, 3DGS의 blob 표현보다 2DGS처럼 표면에 정렬된 표현을 베이스로 고르는 편이 낫겠다는 확신이 들었다.
